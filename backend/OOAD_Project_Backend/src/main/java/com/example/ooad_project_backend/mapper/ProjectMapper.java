...
public interface ProjectMapper extends BaseMapper<ProjectInfo> {
        ...
        @Insert("insert into project(project_title, project_description, project_deadline, project_status, max_score, proportion, releaser, releaser_type, course_id, project_start_date, max_people_in_team,file_id,team_deadline) " +
                        "VALUES (#{projectTitle}, #{projectDescription}, #{projectDeadline}, #{projectStatus}, #{maxScore}, #{proportion}, #{releaser}, #{releaserType}, #{courseId}, #{projectStartDate}, #{maxPeopleInTeam},#{fileId},#{teamDeadline})")
